class ArrivalTime {
  ...
  static getDifferenceMinutes (time) {
    return Math.round(((this.getDifferenceMilliseconds(time) % 86400000) % 3600000) / 60000);
  }
  static getDifferenceHours (time) {
    return Math.floor((this.getDifferenceMilliseconds(time) % 86400000) / 3600000);
  }
  ...
  static updateArrivalTime ($arrivalTimeTableColumn, guestArrivalTimeParsed) {
    ...
    if (passedTime.days > 0) {
      ...
      if (passedTime.minutes > 15) { this.addDangerClass($arrivalTimeTableColumn); }
